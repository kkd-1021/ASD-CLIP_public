...
import clip
from models.classification_net import Classification_net,Classification_net_multi
class XCLIP(CLIP):
    def __init__(self,
                 embed_dim: int,
                 # vision
        ...
        '''分类网络参数设置'''
        #self.classification_net=Classification_net(in_features=768*2,hidden_size=128)
        self.classification_net = Classification_net_multi(in_features=768 * 2)
    ...
    def forward(self, image, text=None, val=True):
        ...
        if val==True:
            ...
            cls_output,sa,rbb = self.classification_net.forward(video_features[0].reshape(-1))
            return cls_output
        ...
        text_features = text_features.reshape(-1, 2, 768)
        text_features = text_features.view(1, -1, 768 * 2)

        video_features = video_features / video_features.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)

        # print("\033[0;31;40mvideo text feature size\033[0m")
        ...
        # print("\033[0;31;40mcls_output_index\033[0m")
        # print(cls_output)
        return logits,cls_output,sa,rbb
