...
Autism-CLIP is a novel machine learning model designed to classify 3-minute videos into Autism Spectrum Disorder (ASD) or non-ASD categories.

# Environment Setup (CUDA 11.3 Required)
To set up the required environment, execute the following commands:
```
git clone https://github.com/kkd-1021/Autism-CLIP_public
cd Autism-CLIP_public
conda create -n Autism-CLIP python=3.8
conda init bash && source /root/.bashrc
conda activate Autism-CLIP
...
Please prepare the pretrained checkpoint `./ckp/k600_14_8.pth`. This project directly loads this checkpoint and does not require running pretrain again.
# Train——finetune
Before training, please ensure that `./ckp/k600_14_8.pth` is prepared. The training process directly loads this checkpoint.
